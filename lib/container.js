...
module.exports = class Container extends EventClose {
    constructor (cluster, options, $event) {
        super();
        this._event = $event;
        this.workers = [];
        this.cluster = cluster;
        this.options = options;
        this.driverManager = options.driverManager;
        ...
        this.log = this.cluster.log.newGroup({
            container: this.id
        });
        ...
        this.onLoaded = cluster.onLoaded
            ...
            .then(() => {
                if (!self.isClosed())
                    return self.cluster._account._manager._vpns.getAvailable();
            ...
            .then((container) => {
                if (!self.isClosed()) {
                    ...
                    self._event.emit('container-created', {
                        id: self.id
                    });
    ...
    newDriver(callback, args) {
        ...
        return this.onLoaded.then(() => {
            ...
            self._event.emit('driver-created', worker.id);
            ...
            return worker.start({
                ...
                .finally(function () {
                    self._event.emit('driver-closed', worker.id);
    ...
    _close() {
        ...
        if (self.container) {
            self.log('stop container');
            return Promise.parse(self.container.stop())
                .finally(() => {
                    self.log('closing container');
                    return self.container.delete({ force: true });
                })
                .finally(() => {
                    self._event.emit('container-closed', {
                        id: self.id
                    });
                    self.log('container closed');
                });
        }
    }
}